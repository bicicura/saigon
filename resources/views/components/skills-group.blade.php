<div class="flex flex-col justify-center gap-8 py-5 text-left lg:p-0 lg:flex-row">
    ...
    <div class="grid w-full grid-cols-2 gap-8 lg:grid-cols-4 lg:gap-0 lg:gap-x-6">
        <div class="flex items-center gap-2 lg:col-span-4">
            <input wire:model="{{$label}}" wire:click="skillClicked('{{$label}}')" class="bg-transparent border-white cursor-pointer lg:order-first border-px focus:ring-0 focus:ring-offset-0" value="1111" type="checkbox" id="no-{{$label}}">
        ...
        @foreach ($skillsgroup as $skill)
            <div class="flex items-center gap-2" wire:key="{{ $loop->index }}">
                ...
                <label for="{{$label}}-{{ $loop->index }}" class="uppercase cursor-pointer">{{__($skill->nombre)}}</label>
            </div>
        @endforeach
    </div>
</div>