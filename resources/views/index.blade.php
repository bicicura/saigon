...
<style>
    ...
    @media (max-width: 800px) {
        .saigon-bg-hero {
            background-image: url(/thumbnails/{{ $Reel->thumbnail }});
        }    
    }
...
<x-desktop-nav-fixed />
{{-- Index Container --}}
<div class="px-3.5 lg:px-8 mt-28 lg:mt-0 {{ $Reel->active? 'space-y-3' : '' }} smooth-scroll lg:fixed" x-data="{ logo: true, reel: false, castings: false }">
    ...
    <section id="castings-comerciales" class="pb-6 saigon-bg-white">
        <article class="space-y-6 text-right text-saigon-white lg:text-left lg:flex lg:flex-col-reverse">
            @if ($Reel->active)
                <div class="relative bg-center bg-no-repeat bg-cover saigon-bg-hero rounded-xl lg:mt-8" x-intersect:enter.threshold.75="reel? '' : logo = false; reel = true; castings = false; $dispatch('indexScrollTo', { vidick: $refs.reel })">
                    <div id="reel" x-ref="reel" class="absolute left-0 -top-24"></div>
                    <a href="{{ route('casting-player', [app()->getLocale(), $Reel->slug]) }}" class="hidden lg:block" data-scroll data-scroll-speed="1.8" href="/">
                        <img class="w-full rounded-xl aspect-video bg-hero-desk" src="/thumbnails/{{ $Reel->thumbnail }}" alt="">
                    </a>
                    <div class="absolute right-3 top-6 lg:hidden">
                        <h1 class="uppercase saigon-text-5xl">{{ $Reel->nombre }}</h1>
            ...
            <div class="space-y-4 text-saigon-black lg:space-y-6" x-intersect:enter.threshold.30="logo? '' : logo = true; reel = false; castings = false; $dispatch('indexScrollTo', { vidick: 'top' })">
                <div class="hidden w-logo-hero lg:block header-logo-big {{ !$Reel->active? 'lg:mt-8' : '' }}">
    ...
    <div class="relative z-20">
        ...
        <div class="mb-12">
            ...
            <div class="flex flex-col items-end gap-4 mt-0 text-right lg:ml-auto lg:gap-4 lg:mt-6" x-intersect:enter.threshold.30="castings? '' : logo = false; reel = false; castings = true; $dispatch('indexScrollTo', { vidick: $refs.comerciales })">
